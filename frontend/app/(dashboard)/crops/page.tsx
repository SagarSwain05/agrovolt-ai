...
import {
    ...
    ArrowRight, Sun, AlertTriangle, XCircle,
} from 'lucide-react';
...
const CROP_DB: Record<string, { shade: number; water: string; minRain: number; price: number; growDays: number }> = {
    Rice: { shade: 0.30, water: 'high', minRain: 800, price: 22, growDays: 150 },
    Tomato: { shade: 0.50, water: 'medium', minRain: 400, price: 35, growDays: 90 },
    Turmeric: { shade: 0.75, water: 'medium', minRain: 600, price: 85, growDays: 240 },
    Ginger: { shade: 0.85, water: 'low', minRain: 500, price: 120, growDays: 210 },
    Spinach: { shade: 0.70, water: 'medium', minRain: 300, price: 40, growDays: 45 },
    Millet: { shade: 0.20, water: 'low', minRain: 300, price: 28, growDays: 120 },
    Groundnut: { shade: 0.35, water: 'medium', minRain: 500, price: 55, growDays: 130 },
    Soybean: { shade: 0.40, water: 'medium', minRain: 600, price: 45, growDays: 110 },
    Wheat: { shade: 0.25, water: 'medium', minRain: 400, price: 25, growDays: 140 },
};

// ICAR Soil × Crop compatibility: 1.0 = ideal, 0.1 = fail
...
// Season → allowed crops (HARD GATE)
...
function clientSideRecommend(soilType: string, rainfall: number, season: string) {
    ...
    const scored = allowedCrops.map(cropName => {
        ...
        // Feature 1: Soil compatibility (30%)
        const soilScore = soilCompat[cropName] || 0.5;

        // Feature 2: Rainfall band scoring (25%)
        let rainScore = 0.5;
        const range = RAIN_RANGES[cropName];
        if (range) {
            if (rainfall < range.min) {
                rainScore = Math.max(0, rainfall / range.min * 0.3);
            } else if (rainfall >= range.optLow && rainfall <= range.optHigh) {
                rainScore = 1.0;
            } else if (rainfall < range.optLow) {
                rainScore = 0.5 + 0.5 * ((rainfall - range.min) / (range.optLow - range.min));
            } else if (rainfall > range.optHigh && rainfall <= range.max) {
                rainScore = 0.5 + 0.5 * ((range.max - rainfall) / (range.max - range.optHigh));
            } else {
                rainScore = Math.max(0, 0.2);
            }
        }

        // Feature 3: Shade tolerance (15%)
        const shadeScore = crop.shade;

        // Feature 4: Market value (10%)
        const marketScore = crop.price / maxPrice;

        // Feature 5: Water efficiency (10%)
        let waterScore = 0.5;
        ...
        // Feature 6: General yield potential (10%)
        const yieldScore = 0.7 + soilScore * 0.3;

        // Weighted total
        const total = soilScore * 0.30 + rainScore * 0.25 + shadeScore * 0.15 + marketScore * 0.10 + waterScore * 0.10 + yieldScore * 0.10;
    ...
    const excluded = allCrops.filter(c => !allowedCrops.includes(c)).map(c => ({
        name: c,
        reason: `Not suitable for ${season} season`,
    }));
...
export default function CropsPage() {
    ...
    const handleRecommend = async () => {
        ...
        setModelSource('');
        try {
            const res = await cropAPI.getRecommendations({ soilType, rainfall: parseInt(rainfall), season });
            setRecommendations(res.data.data?.recommendations || []);
            setExcluded(res.data.data?.excluded || []);
            setModelSource('Server ML (v2.0)');
        } catch {
            // Backend unavailable — run client-side ML engine
            const result = clientSideRecommend(soilType, parseInt(rainfall), season);
            setRecommendations(result.recommendations as any[]);
            setExcluded(result.excluded);
            setModelSource('Client ML (v2.0)');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div>
            <Navbar title="Crop Intelligence" subtitle="AI-powered shade-smart recommendations & lifecycle management" />
            ...
            <div className="page-container">
                ...
                <div className="grid-2" style={{ marginBottom: '1.5rem' }}>
                    ...
                    <div className="card">
                        ...
                        <p style={{ fontSize: '0.75rem', color: 'var(--color-gray-400)', marginBottom: '1.25rem' }}>
                            AI selects crops that thrive under solar panel shadow patterns
                        </p>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                            ...
                            <button onClick={handleRecommend} className="btn-primary" disabled={loading} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                                <Search size={16} /> {loading ? 'Analyzing...' : 'Get AI Recommendations'}
                            </button>
                        </div>
                    </div>
                                ...
                                {recommendations.map((crop, i) => (
                                    <div key={crop.name} style={{
                                        ...
                                        <div style={{ height: '4px', background: 'var(--color-gray-100)', borderRadius: '2px', marginBottom: '0.5rem' }}>
                                            <div style={{
                                                height: '100%',
                                                width: `${(crop.confidence || crop.successRate / 100) * 100}%`,
                                                borderRadius: '2px',
                                        ...
                                        {/* Soil & Rainfall match badges */}
                                        {(crop.soilMatch || crop.rainfallMatch) && (
                                            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.375rem' }}>
                                                {crop.soilMatch && (
                                                    <span style={{
                                                        fontSize: '0.5625rem', padding: '0.125rem 0.375rem', borderRadius: '4px', fontWeight: 600,
                                                        background: crop.soilMatch === 'Excellent' ? '#dcfce7' : crop.soilMatch === 'Good' ? '#f0fdf4' : crop.soilMatch === 'Fair' ? '#fefce8' : '#fef2f2',
                                                        color: crop.soilMatch === 'Excellent' ? '#166534' : crop.soilMatch === 'Good' ? '#15803d' : crop.soilMatch === 'Fair' ? '#a16207' : '#991b1b',
                                                    }}>
                                                        Soil: {crop.soilMatch}
                                                    </span>
                                                )}
                                                {crop.rainfallMatch && (
                                                    <span style={{
                                                        fontSize: '0.5625rem', padding: '0.125rem 0.375rem', borderRadius: '4px', fontWeight: 600,
                                                        background: crop.rainfallMatch === 'Excellent' ? '#dbeafe' : crop.rainfallMatch === 'Good' ? '#eff6ff' : crop.rainfallMatch === 'Fair' ? '#fefce8' : '#fef2f2',
                                                        color: crop.rainfallMatch === 'Excellent' ? '#1e40af' : crop.rainfallMatch === 'Good' ? '#1d4ed8' : crop.rainfallMatch === 'Fair' ? '#a16207' : '#991b1b',
                                                    }}>
                ...
                <div className="card">
                    <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.125rem', fontWeight: 700, color: 'var(--color-gray-800)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        Growth Lifecycle — Turmeric (Day 45)
                    </h3>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', overflowX: 'auto', paddingBottom: '0.5rem' }}>
                        {[
                            { stage: 'Seed Prep', days: 'Day 1-7', done: true },
                            { stage: 'Planting', days: 'Day 8-14', done: true },
                            { stage: 'Sprouting', days: 'Day 15-30', done: true },
                            { stage: 'Vegetative', days: 'Day 31-90', done: false, current: true },
                            { stage: 'Flowering', days: 'Day 91-150', done: false },
                            { stage: 'Maturity', days: 'Day 151-240', done: false },
                            { stage: 'Harvest', days: 'Day 241+', done: false },
                        ].map((phase, i) => (
                            <div key={phase.stage} style={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>
                                <div style={{
                                    ...
                                    borderRadius: 'var(--radius-xl)',
                                    background: phase.current ? 'var(--color-green-50)' : phase.done ? 'white' : 'var(--color-gray-50)',
                                    border: phase.current ? '2px solid var(--color-green-400)' : '1px solid var(--color-gray-200)',
                                }}>
                                    <div style={{ marginBottom: '0.25rem' }}>
                                        {phase.done ? <CheckCircle2 size={18} color="var(--color-green-500)" /> : phase.current ? <Sprout size={18} color="var(--color-green-600)" /> : <div style={{ width: '18px', height: '18px', borderRadius: '50%', border: '2px solid var(--color-gray-300)', margin: '0 auto' }} />}
                                    </div>
                                    <div style={{ fontSize: '0.75rem', fontWeight: 600, color: phase.current ? 'var(--color-green-700)' : 'var(--color-gray-700)' }}>{phase.stage}</div>
                                    <div style={{ fontSize: '0.625rem', color: 'var(--color-gray-400)', fontFamily: 'var(--font-mono)' }}>{phase.days}</div>
                                </div>
